...
const randomP = new Promise<number>((resolve) =>
...
Promise.all([simulateTask(500), simulateTask(1000), simulateTask(1500)]).then(
  (res) => console.log("6:", res)
);
// 7.  Use  Promise.race() to return  whichever  Promise  resolves  first.
Promise.race([simulateTask(500), simulateTask(1000)]).then((r) =>
  console.log("7:", r)
);
...
new Promise<number[]>((resolve) =>
  setTimeout(() => resolve(nums.filter((n) => n % 2 === 0)), 1000)
).then((res) => console.log("9:", res));
